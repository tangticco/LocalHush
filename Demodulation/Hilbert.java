...
public class Hilbert {
    ...
    public static Complex[] transform(double[] x, int N) {

        Complex[] xComplex = Library.doubleArray2ComplexArray(x);
        Complex[] X = FFT.fft(xComplex);
        ...
        Complex next;
        for (w = 0; w < N; w++) {
            // Big opportunity for speed savings right here
            next = new Complex(X[w].re() * H[w], X[w].im() * H[w]);
            X[w] = next;
            //X.real[w] *= H[w];
            //X.imag[w] *= H[w];
        }
